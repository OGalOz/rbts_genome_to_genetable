...
def genbank_and_genome_fna_to_gene_table(gbk_fp, gnm_fp, op_fp):
    ...
    *** We only write features whose type is 'CDS' to the output file,
    ...
    # This dict counts how many times each feature name from
    # above types dict was found in the file.
    type_count_d = {}
    ...
    for gb_record in gb_record_generator:
        ...
        for i in range(g_feat_len):
            ...
            current_seq = g_seq[
                current_feat.location.start : current_feat.location.end
            ].upper()
            ...
            if typ_str in types_dict:
                typ_num = str(types_dict[typ_str])
            else:
...
def findSeqInId2Seq(seq_str, id2seq):
    ...
    if seq_id is None:
        raise Exception(
            "Couldn't find Sequence ID for sequence:\n" + f" '{seq_str}'\n\nin id2seq"
        )
...
def unduplicate_gene_table(gene_table_string):
    ...
    print(splitLine)
    existing_loc = splitLine[1:3]
    existing_typ = splitLine[2]
    previous_index = 0
    ...
    for i in range(1, len(gt_lines) - 1):
        ...
        print(i)
        current_loc = splitLine[1:3]
        crnt_typ = splitLine[-1]
        if (current_loc[0] == existing_loc[0]) or (current_loc[1] == existing_loc[1]):
            if crnt_typ == "1":
                if existing_typ == "1":
                    logging.warning(
                        "Two overlapping location Protein "
    ...
    logging.info(
        "New total line number (after duplicate line removal): " + str(len(gt_lines))
    )
...
def test(args):
    ...
    OLD_convert_genbank_to_gene_table(gb_fp, op_fp)
...
def OLD_convert_genbank_to_gene_table(
    genbank_filepath, output_filepath, gff_fasta_gbk=True, config_dict={}
):
        ...
        for i in range(g_feat_len):
            ...
            if "product" in current_feat.qualifiers.keys():
                ...
                if gff_fasta_gbk and "locus_tag" in current_feat.qualifiers:
                    desc = " ".join(
                        current_feat.qualifiers["locus_tag"][0].split(" ")[1:]
                    ).split("=")[-1]
                else:
            ...
            if typ_str in types_dict:
                ...
                logging.info("Could not recognize type from feature: " + typ_str)
                typ = "0"
            if typ == "1":
                out_FH.write(
                    "\t".join(
